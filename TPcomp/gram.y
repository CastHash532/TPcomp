...
%token TOKEN_FLOAT TOKEN_INTEGER TOKEN_STRINGCONST TOKEN_ARITHMATICOP TOKEN_NOTEQUALOP TOKEN_ASSIGNOP TOKEN_RIGHTPAREN TOKEN_LEFTPAREN TOKEN_RB TOKEN_LB
...
%type<xint> TOKEN_INTEGER
%type<xfloat> TOKEN_FLOAT
%type<chaine> names
%type<chaine> type
...
| ;
define:   define TOKEN_LET TOKEN_ID TOKEN_BE TOKEN_FLOAT TOKEN_ENDINSTR {
	/* Regles Symantiques    ***************** ICI *************** */
			if(recherche(tete,$3)==1) {
				printf("define float avec succes ligne %d\n", lineno);
				tete=inser(tete,$3,"float",$5);
				affiche_liste(tete);
				}
			else printf(" %s: nom de variable déja utilisé\n",$3);
		}

        |   define TOKEN_LET TOKEN_ID TOKEN_BE TOKEN_INTEGER TOKEN_ENDINSTR {
			/* Regles Symantiques    ***************** ICI *************** */
			if(recherche(tete,$3)==1) {
				printf("define integer avec succes ligne %d\n", lineno);
				tete=inser(tete,$3,"integer",$5);
				affiche_liste(tete);
				}
			else printf(" %s: nom de variable déja utilisé\n",$3);
		}
		...
		|   
;
number_const: TOKEN_FLOAT | TOKEN_INTEGER;
...
for_instruction: TOKEN_FROM TOKEN_ID TOKEN_EQUALOP number_const  TOKEN_TO TOKEN_ID TOKEN_EQUALOP number_const TOKEN_WITH TOKEN_ID TOKEN_ADD TOKEN_ADD TOKEN_LB corps TOKEN_RB ;
